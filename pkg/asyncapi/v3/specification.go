...
// DependencySchemas returns the schemas defined in the specification's
// dependencies that are not already generated by the main specification (keyed
// by their generated type name). This lets schemas referenced from another file
// — a shared schema file or an OpenAPI document used as a dependency — be
// generated, while avoiding duplicate declarations when the main spec defines a
// schema with the same name.
func (s Specification) DependencySchemas() map[string]*Schema {
	// Names already generated by the main specification's own components.
	used := make(map[string]struct{})
	for _, schema := range s.Components.Schemas {
		used[schema.Name] = struct{}{}
	}

	schemas := make(map[string]*Schema)
	for _, dep := range s.dependencies {
		for _, schema := range dep.Components.Schemas {
			if _, exists := used[schema.Name]; exists {
				continue
			}
			used[schema.Name] = struct{}{}
