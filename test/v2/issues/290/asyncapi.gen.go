// Package "issue290" provides primitives to interact with the AsyncAPI specification.
//
...
// PublishTestCreated will publish messages to 'test.created' channel
func (c *AppController) PublishTestCreated(
	ctx context.Context,
	msg TestCreatedMessage,
) error {
	// Get channel path
	path := "test.created"
...
type UserSubscriber interface {
	// TestCreated subscribes to messages placed on the 'test.created' channel
	TestCreated(ctx context.Context, msg TestCreatedMessage) error
}
...
func (c *UserController) SubscribeAll(ctx context.Context, as UserSubscriber) error {
	...
	if err := c.SubscribeTestCreated(ctx, as.TestCreated); err != nil {
		return err
	}
...
func (c *UserController) UnsubscribeAll(ctx context.Context) {
	c.UnsubscribeTestCreated(ctx)
}

// SubscribeTestCreated will subscribe to new messages from 'test.created' channel.
//
// Callback function 'fn' will be called each time a new message is received.
func (c *UserController) SubscribeTestCreated(
	ctx context.Context,
	fn func(ctx context.Context, msg TestCreatedMessage) error,
) error {
	// Get channel path
	path := "test.created"
...
func (c *UserController) listenToTestCreatedNextMessage(
	path string,
	...
	if err := c.executeMiddlewares(msgCtx, &acknowledgeableBrokerMessage.BrokerMessage, func(middlewareCtx context.Context) error {
		// Process message
		msg, err := brokerMessageToTestCreatedMessage(acknowledgeableBrokerMessage.BrokerMessage)
		if err != nil {
			return err
		}
...
// UnsubscribeTestCreated will unsubscribe messages from 'test.created' channel.
// A timeout can be set in context to avoid blocking operation, if needed.
func (c *UserController) UnsubscribeTestCreated(ctx context.Context) {
	// Get channel path
	path := "test.created"
...
// TestCreatedMessage is the message expected for 'TestCreatedMessage' channel.
type TestCreatedMessage struct {
	// Payload will be inserted in the message payload
	Payload TestEventSchema
}

// NewTestCreatedMessage creates a new TestCreatedMessage with default values if any.
func NewTestCreatedMessage() TestCreatedMessage {
	var msg TestCreatedMessage

	return msg
}

// brokerMessageToTestCreatedMessage will fill a new TestCreatedMessage with data from generic broker message
func brokerMessageToTestCreatedMessage(bMsg extensions.BrokerMessage) (TestCreatedMessage, error) {
	var msg TestCreatedMessage
...
// toBrokerMessage will generate a generic broker message from TestCreatedMessage data
func (msg TestCreatedMessage) toBrokerMessage() (extensions.BrokerMessage, error) {
...
// TestEventSchema is a schema from the AsyncAPI specification required in messages
type TestEventSchema struct {
...
const (
	// TestCreatedPath is the constant representing the 'TestCreated' channel path.
	TestCreatedPath = "test.created"
)
