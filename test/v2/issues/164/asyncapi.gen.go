...
type AppSubscriber interface {
	...
	Issue164TestMap(ctx context.Context, msg TestMapMessage)
}
...
func (c *AppController) SubscribeIssue164TestMap(ctx context.Context, fn func(ctx context.Context, msg TestMapMessage)) error {
	// Get channel path
	...
	go func() {
		for {
			...
			if err := c.executeMiddlewares(ctx, &brokerMsg, func(ctx context.Context) error {
				// Process message
				msg, err := newTestMapMessageFromBrokerMessage(brokerMsg)
				if err != nil {
					return err
				}
...
func (c *UserController) PublishIssue164TestMap(ctx context.Context, msg TestMapMessage) error {
	// Get channel path
	path := "issue164.testMap"
