...
func (c *AppController) PublishTest2(msg Test2Message) error {
	...
	return c.brokerController.Publish("test2", um)
}
...
func (c *ClientController) UnsubscribeTest2() {
	stopChan, exists := c.stopSubscribers["test2"]
	if !exists {
		return
	}

	stopChan <- true
	delete(c.stopSubscribers, "test2")
}
