// Package "issue139" provides primitives to interact with the AsyncAPI specification.
//
...
// SendAsSendUserOperation will send a UserSignedUp message on User channel.
//
// NOTE: for now, this only support the first message from AsyncAPI list.
// If you need support for other messages, please raise an issue.
func (c *AppController) SendAsSendUserOperation(
	ctx context.Context,
	params UserChannelParameters,
...
type UserSubscriber interface {
	// SendUserOperationReceived receive all UserSignedUp messages from User channel.
	SendUserOperationReceived(ctx context.Context, msg UserSignedUpMessage) error
}
...
// SubscribeToSendUserOperation will receive UserSignedUp messages from User channel.
//
...
// NOTE: for now, this only support the first message from AsyncAPI list.
// If you need support for other messages, please raise an issue.
func (c *UserController) SubscribeToSendUserOperation(
	ctx context.Context,
	params UserChannelParameters,
...
func (c *UserController) listenToSendUserOperationNextMessage(
	addr string,
	...
	if err := c.executeMiddlewares(msgCtx, &acknowledgeableBrokerMessage.BrokerMessage, func(middlewareCtx context.Context) error {
		// Process message
		msg, err := brokerMessageToUserSignedUpMessage(acknowledgeableBrokerMessage.BrokerMessage)
		if err != nil {
			return err
		}
...
// UnsubscribeFromSendUserOperation will stop the reception of UserSignedUp messages from User channel.
// A timeout can be set in context to avoid blocking operation, if needed.
func (c *UserController) UnsubscribeFromSendUserOperation(
	ctx context.Context,
	params UserChannelParameters,
) {
	// Get channel address
...
// UserChannelParameters represents UserChannel channel parameters
type UserChannelParameters struct {
	// UserId is a channel parameter.
	UserId string
}

// Message 'UserSignedUpMessageFromUserChannel' reference another one at '#/components/messages/userSignedUp'.
...
const (
	// UserChannelPath is the constant representing the 'UserChannel' channel path.
	UserChannelPath = "v3.issue139.user.{userId}"
)
