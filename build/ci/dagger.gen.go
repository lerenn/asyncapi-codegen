...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "AsyncapiCodegenCi":
		...
		case "Check":
			...
			return (*AsyncapiCodegenCi).Check(&parent, ctx, dir)
		case "Generate":
			var parent AsyncapiCodegenCi
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
		...
		case "Lint":
			...
			return (*AsyncapiCodegenCi).Lint(&parent, ctx, dir)
		case "Examples":
			var parent AsyncapiCodegenCi
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*AsyncapiCodegenCi).Examples(&parent, ctx, dir)
		case "Tests":
			var parent AsyncapiCodegenCi
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*AsyncapiCodegenCi).Tests(&parent, ctx, dir)
		case "Publish":
			var parent AsyncapiCodegenCi
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return nil, (*AsyncapiCodegenCi).Publish(&parent, ctx, dir, tag)
		default:
	...
	case "":
		return dag.Module().
			...
			WithObject(
				dag.TypeDef().WithObject("AsyncapiCodegenCi", TypeDefWithObjectOpts{Description: "AsyncapiCodegenCi is the Dagger CI module for AsyncAPI Codegen."}).
					WithFunction(
						dag.Function("Check",
					...
					WithFunction(
						dag.Function("Generate",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("Generate files from Golang generate command on AsyncAPI-Codegen source code.").
							WithArg("dir", dag.TypeDef().WithObject("Directory"))).
					WithFunction(
						dag.Function("Lint",
							dag.TypeDef().WithKind(StringKind)).
					...
					WithFunction(
						dag.Function("Publish",
							...
							WithArg("dir", dag.TypeDef().WithObject("Directory")).
							WithArg("tag", dag.TypeDef().WithKind(StringKind)))), nil
	default:
